...
def demografía_y_firmas(estado):
    ...
    ancla_trabajadores = min(config.num_trabajadores / num_trabajadores, ratio)
    ancla_empresas = min(config.num_empresas / num_empresas, 1)
    ...
    cantidad_trabajadores_base = min(round(exceso_trabajadores * 5), 2)
    cantidad_empresas_base = min(round(exceso_empresas * 5), 2)

    cantidad_trabajadores = int(cantidad_trabajadores_base * ancla_trabajadores)
    cantidad_empresas = int(cantidad_empresas_base * ancla_empresas)
    ...
    if beneficio_esperado > beneficio_ref * 1.1:
        for _ in range(cantidad_empresas):
            ...
            nueva_empresa.probabilidad_venta_esperada = estado.probabilidad_venta_esperada_medio
            nueva_empresa.beneficio_esperado = estado.beneficio_esperado_medio
